...
/** Keeps the Telegram token on the server; the browser only calls this route. */
function telegramApplications(token: string | undefined, chatId: string | undefined): Plugin {
  return {
    name: 'telegram-applications',
    configureServer(server) {
      server.middlewares.use('/api/applications', async (req, res, next) => {
        if (req.method !== 'POST') return next()

        try {
          const chunks: Buffer[] = []
          for await (const chunk of req) chunks.push(Buffer.from(chunk))
          const application = JSON.parse(Buffer.concat(chunks).toString('utf8')) as Application
          const name = text(application.name, 120)
          const phone = text(application.phone, 60)
          const messenger = text(application.messenger, 40) || '—'
          const comment = text(application.comment, 2000) || '—'
          ...
          if (!token || !chatId) {
            console.error('Telegram is not configured: TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID are required.')
            res.statusCode = 503
            res.end(JSON.stringify({ error: 'Delivery is not configured' }))
            return
          }
          ...
          const message = [
            ...
            `Комментарий: ${comment}`,
            `Язык сайта: ${text(application.language, 10) || '—'}`,
            `Время (UTC): ${text(application.submittedAt, 40) || '—'}`,
            `Страница: ${text(application.pageUrl, 1000) || '—'}`,
          ].join('\n')
          ...
          res.statusCode = 502
          res.end(JSON.stringify({ error: 'Could not deliver application' }))
        }
      })
    },
  }
...
react(),
      ...
      telegramApplications(env.TELEGRAM_BOT_TOKEN, env.TELEGRAM_CHAT_ID),
